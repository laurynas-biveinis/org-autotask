...
** Archiving Old Clock Entries

The command ~org-autotask-clock-archive-old~ archives CLOCK entries older than a
configurable threshold from the current Org file to its standard archive file (as
defined by ~org-archive-location~).

Usage:
- ~M-x org-autotask-clock-archive-old~ - Archive clocks older than
  ~org-autotask-clock-archive-default-days~ days (default: 365)
- ~C-u M-x org-autotask-clock-archive-old~ - Prompt for the threshold in days

Archived clocks are placed under headings tagged with ~:archived_clocks:~.  This
tag is used instead of Org's standard ~:ARCHIVE:~ tag because we archive only
clock entries, not entire tasks—the original task may still be active.  The
~:archived_clocks:~ tag is purely organizational and does not affect clock
