...
The fourth ~org-stuck-projects~ element is a regexp that marks a project as
/not/ stuck when its subtree contains a next action (the next-action keyword)
tagged with one of the contexts. Because projects and next actions share the
...
stuck). A project whose only descendant is a waiting-for item, a contextless
action, or a completed action therefore stays stuck.
