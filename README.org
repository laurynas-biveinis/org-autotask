...
A project counts as /not/ stuck when its subtree contains an actionable next
action: a heading with the next-action keyword and one of the context tags,
whose someday/maybe tag is /not/ set (locally or inherited from a
someday/maybe pocket ancestor). Because projects and next actions share the
next-action keyword, this cannot be the keyword-based not-stuck test (which
would match the project's own headline and so never report any project as
stuck); and because a someday/maybe tag inherited from an ancestor never
appears on the incubated action's own headline line, no line-local regexp can
exclude it either. ~org-autotask-initialize~ therefore leaves the fourth
~org-stuck-projects~ element as a never-matching regexp and installs the
tag-aware not-stuck test as advice on ~org-agenda-list-stuck-projects~. A
project whose only descendant is a waiting-for item, a contextless action, a
completed action, or a someday/maybe-incubated next action therefore stays
stuck.
...
The command ~org-autotask-log-archive-old~ archives CLOCK entries and state
change entries older than a configurable threshold from the current Org file to
its standard archive file (as defined by ~org-archive-location~).

Usage:
- ~M-x org-autotask-log-archive-old~ - Archive log entries older than
  ~org-autotask-log-archive-default-days~ days (default: 365)
- ~C-u M-x org-autotask-log-archive-old~ - Prompt for the threshold in days

Archived entries are placed under headings tagged with ~:archived_logs:~.  This
tag is used instead of Org's standard ~:ARCHIVE:~ tag because we archive only
log entries, not entire tasks—the original task may still be active.  The
~:archived_logs:~ tag is purely organizational and does not affect clock
reporting.  Following Org's archiving convention, each entry includes
~ARCHIVE_OLPATH~ and ~ARCHIVE_FILE~ properties to preserve context:

#+BEGIN_SRC org
,* Task Name :archived_logs:
:PROPERTIES:
:ARCHIVE_OLPATH: Project/Subproject
:ARCHIVE_FILE: /path/to/source.org
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2023-01-15 Sun 08:30]
CLOCK: [2023-01-15 Sun 09:00]--[2023-01-15 Sun 10:30] =>  1:30
:END:
#+END_SRC

Running clocks and incomplete clock entries are never archived.
